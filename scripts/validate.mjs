...
import {teachingTables,teachingFor,renderTeaching,knowledgePoints} from '../dist/teaching.js';
...
for(const id of Object.keys(teachingTables))assert(unitById[id],`Orphan teaching table: ${id}`);
for(const u of units){
 const d=getDiagram(u);if(d)assert(d.includes('<svg')&&d.includes('</svg>'),`Broken svg diagram for ${u.id}`);
 assert(teachingTables[u.id],`Missing authored table for ${u.id}`);
 const t=teachingFor(u);assert(t.rows.length>=3,`Too few teaching rows: ${u.id}`);
 for(const row of t.rows){assert.equal(row.length,3);for(const cell of row)assert(typeof cell==='string'&&cell.length>0,`Empty cell: ${u.id}`)}
 assert(knowledgePoints(u).length>0);assert(renderTeaching(u).includes('<caption>'));
}
...
const summary={subjects:subjects.length,domains:domains.length,units:units.length,questions:all.length,lessonQuestions:questions.length,extraQuestions:extraQuestions.length,listening:listening.length,constructed:constructed.length,teachingTables:Object.keys(teachingTables).length,teachingRows:units.reduce((sum,u)=>sum+teachingFor(u).rows.length,0),specializedDiagrams:units.filter(u=>getDiagram(u)).length};
